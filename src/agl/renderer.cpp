...
void Renderer::loadTexture(const std::string& name,
    const vector<float> image, int slot) {
  ...
  if (_textures.count(name) == 0) {
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image.size(), 1, 0, GL_RGBA, GL_FLOAT, image.data());
   //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image.size(), 1, GL_RGBA, GL_FLOAT, image.data());
